...
class PluginRegistry:
    ...
    def auto_discover(cls):
        ...
        try:
            ...
            for _, module_name, ispkg in pkgutil.iter_modules(causalnerve.plugins.__path__):
                try:
                    if ispkg:
                        mod = importlib.import_module(f"causalnerve.plugins.{module_name}.plugin")
                    else:
                        mod = importlib.import_module(f"causalnerve.plugins.{module_name}")
