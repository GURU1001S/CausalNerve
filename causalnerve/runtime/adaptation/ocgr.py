"""
causalnerve.runtime.adaptation.ocgr
======================
...
from causalnerve.runtime.safety.sufficiency import CausalSufficiencyChecker, DelayedConfirmationGate, AdaptiveAlarmThreshold
from .live_validation import LiveSurgeryValidator
from .surgery_validator import MonteCarloSurgeryValidator, SurgeryValidationResult
...
class GraphSurgeryEngine:
    ...
    Validation is performed by :class:`MonteCarloSurgeryValidator`
    which runs N dual-world stochastic rollouts.  No heuristic
    leakage scaling is used — every metric emerges from computation.
    """
    def __init__(self,
                 graph: CausalGraphEngine,
        ...
        self.mc_validator = MonteCarloSurgeryValidator(
            horizon=25, n_rollouts=16, seed=42
        )
        # Cached last validation detail for apply()
        self._last_mc_result: Optional[SurgeryValidationResult] = None
    ...
    def propose(self, alarm: AlarmEvent, current_state: Any = None) -> List[EditProposal]:
        ...
        # Emergency detection based on leakage magnitude
        is_emergency = alarm.leakage_value > 0.50

        # Priority from fleet recurrence priors (populated externally)
        is_priority = (src, tgt) in getattr(self, 'fleet_priors', set())

        # Base confidence proportional to leakage severity
        base_conf = min(0.9, alarm.leakage_value * 2.0)

        proposals.append(
            EditProposal(
                edge=(src, tgt),
                edit_type="add_or_remove",
    ...
    def validate(self,
        ...
        """
        Validate a proposed surgery using Monte-Carlo dual-world rollouts.

        No hardcoded leakage scaling.  The MonteCarloSurgeryValidator
        runs N stochastic simulations on the original and edited graphs
        and computes real leakage integrals, Lyapunov energy deltas,
        and divergence metrics.
        """
        # Convert torch adjacency to numpy for the MC validator
        adj_np = current_graph.adj.detach().cpu().numpy().astype(np.float64)

        # Run real dual-world Monte-Carlo validation
        mc = self.mc_validator.validate(
            adj=adj_np,
            state_vector=state_vector.astype(np.float64),
            edge=proposal.edge,
            edit_type=proposal.edit_type,
        )
        self._last_mc_result = mc

        # Map MC result onto the existing ValidationResult dataclass
        # so callers (OCGROrchestrator, LiveSurgeryValidator) are unaffected.
        V_before = mc.leakage_before   # Energy proxy: factual leakage integral
        V_after = mc.leakage_after     # Energy proxy: edited leakage integral
        confidence = mc.confidence

        # Override proposal confidence with the rollout-derived value
        proposal.predicted_confidence = confidence

        accepted = mc.accepted and confidence >= self.confidence_threshold
        reason = mc.reason if mc.accepted else mc.reason
        if not accepted and confidence < self.confidence_threshold:
            reason = f"Low MC confidence ({confidence:.2f} < {self.confidence_threshold})"
    ...
    def apply(self,
        ...
        """
        Record a revision event.  ``leakage_after`` is taken from the
        Monte-Carlo validation result — not from a hardcoded multiplier.
        """
        # Use the real MC-computed leakage_after if available
        mc = self._last_mc_result
        real_leakage_after = mc.leakage_after if mc is not None else validation.V_after
        ...
        event = RevisionEvent(
            ...
            leakage_after=real_leakage_after,
    ...
    def step(self, 
        ...
        for alarm in alarms:
            ...
            for p in proposals:
                ...
                # Use real MC-computed leakage from the surgery validator
                leakage_after = val_result.V_after  # V_after now holds MC leakage_after
